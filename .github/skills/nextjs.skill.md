...
A request handler must never block on genuinely CPU/latency-heavy work (>500ms — see
`#file:.github/skills/engineering-standards.skill.md`'s Scalability gate). If that work is
Python-portable, move it to `fastapi-route.agent.md` instead. If it depends on a **Node-only
library with no Python port** (e.g. `@react-pdf/renderer` — no FastAPI equivalent exists without
rewriting the whole template in a different language, which is its own kind of scope creep), it
has to stay in Next.js — but it still can't run inline. Use Next's `after()` (stable since 15.1,
`import { after } from 'next/server'`) to schedule the work *after* the response is sent, backed
by a small DB-tracked job-status row the client polls:
