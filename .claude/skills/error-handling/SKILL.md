---
name: error-handling
description: Side-by-side TypeScript and Python error handling — typed Result pattern, custom error/exception hierarchies, async error rules (never swallow), structured logging (pino/structlog) with redaction and request-correlation IDs, Sentry/APM error tracking, and the Next.js error boundaries. Use when handling failures, adding logging, or wiring error tracking.
---
