...
3. If touching the Checkout route: verify the duplicate-subscription guard and the
   create-race handling are still present after your change (see the `payments` skill).
...
From this repo's first Stripe integration and its security audit — see the `payments` skill for
