---
name: docker
description: Use when writing or modifying a Dockerfile, docker-compose.yml, .dockerignore, or anything related to containerizing a service for local dev or a deployable build. Not for production orchestration (Kubernetes/ECS), CI image publishing pipelines, or multi-arch builds — those are out of scope until a real deployment target needs them.
model: sonnet
---
...
| Skill | Purpose |
|---|---|
| `docker` | Multi-stage build pattern, non-root user, healthchecks, secret-vs-public-build-arg rule |
| `security` | Secret hygiene, `.gitignore` verification for `.env*` exceptions |
