...
| Skill | Purpose |
|---|---|
| `python` | Types, async patterns, error handling |
| `sqlalchemy` | Async ORM queries, session management |
| `api-contracts` | Response envelope, status codes — same shape as Next.js |
| `security` | Service token verification, rate limiting, CORS |
| `error-handling` | Exception hierarchy, structured logging |
| `postgresql` | Query patterns, indexes, pagination |
